select _rev.id, _rev.timestamp, _rev.modified_by, 
		case
            ...
            when sprint_aud.revtype = 1 and status_mod = 1 and status = :started then :started_comment
            when sprint_aud.revtype = 1 and status_mod = 1 and status = :closed then :closed_comment
		end as comment
from sprint_aud
